...
const List = () => {
  return (
    <div className="Main-events p-2">
      <h5 className="text-warning mb-3 fw-bold">Presentations at Conferences and Seminars</h5>
      <Table striped bordered hover variant="dark">
        <thead>
          <tr className="table-secondary text-uppercase fs-7">
            <th>Event</th>
            <th>Location</th>
            <th>Date</th>
          </tr>
        ...
        <tbody>
          {events.map((item) => (
            <tr key={item.name}>
              ...
              <td className="text-info font-monospace">{item.date}</td>
            </tr>
          ))}
        </tbody>
      </Table>
